    ...
    if($result=$mysqli->query($qury)){
        if(mysqli_num_rows($result)>0){
            ...
            if($due > 0){
                ...
                $_SESSION['null-nonthly-due'] = $name." has not any due in the month of ".month($month).",".$year.".";
                header('location:customer-main-page.php');
            }
        }
        else{
            $_SESSION['null-monthly-due'] = $name." has not any due in the month of ".month($month).",".$year.".";
            header('location:customer-main-page.php');
        }
    }
    mysqli_close($mysqli);
