<?php
    ...
    $date = $_POST['date'];
    $qury = "SELECT date FROM today_total_newspaper WHERE date = '".$date."'";
    if($result = $mysqli->query($qury)){
        if(mysqli_num_rows($result)>0){
            ...
            $_SESSION['no-details-newspaper'] = "There are no newspaper entry on ".$date.".";
    ...
    function display($date){
        ...
        <body>
            ...
            <div class="text-center">
                <h2 class="text-danger">Today Total Newspaper View</h2>
            </div><br><br>
            ';
            $date = $_POST['date'];
            $qury = "SELECT * FROM today_total_newspaper WHERE date = '".$date."'";
            echo '
                        ...
                        if(mysqli_num_rows($result)>0) {
                            while($row = $result->fetch_assoc()) {
                                ...
                                <thead>
                                    <tr>
                                        <th scope="col">'.$date.'</th>
                                        <th scope="col">'.$newspaper_name.'</th>
                                        <th scope="col">'.$newspaper_price.'</th>
                                        <th scope="col">'.$newspaper_quentity.'</th>
                                        <th scope="col">'.$percentage.'</th>
