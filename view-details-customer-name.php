<?php  
    ...
    $mobile = $_POST['mobile'];
    $qury = "SELECT name FROM customer_entry WHERE name = '".$customername."' and mobile = '".$mobile."'";
    if($result = $mysqli->query($qury)){
        if(mysqli_num_rows($result)>0){
            display($customername,$mobile);
        }
    }

    function display($customername,$mobile){
        require_once 'database-connection.php';
        $mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_DATABASE);
        $qury = "SELECT * FROM customer_entry WHERE name = '".$customername."' and mobile = '".$mobile."'";
        if($result = $mysqli->query($qury)){
            if(mysqli_num_rows($result)>0){
                ...
                echo '
                    ...
                    <head>
                        ...
                        <title>Customer Details Check</title>
                    ...
                    <body>
                        ...
                        <div class="text-center">
                            <h2 class="text-danger">Customer Details View</h2>
                        </div><br><br>
