<?php
    ...
    $qury = "SELECT newspaper_name,regular_price,sunday_price FROM newspaper_entry";
    $regular_price = array();
    $sunday_price = array();
    echo '
        <!doctype html>
        ...
        <head>
            ...
            <title>Today Total Newspaper</title>
        ...
        <br><br>
            ...
            <div class="text-center">
                <h2 class="text-danger">Today Total Newspaper</h2>
            </div>
            <div class="text-danger ml-4">
                <p>Mandatory fields are marked with an asterisk(*)</p>
            </div>

            <form action="employee-input-today-total-newspaper.php" method="post">
                <div class="row mx-5 p-3" style="background-color: rgb(255, 187, 0); border-radius: 25px;">
                    ...
                    <div class="col-lg-2 col-md-4 pt-3 mb-1">
                        <label for="papername" class="mb-0"><b>Newspaper Name</b><span class="text-danger">*</span></label>
                        <select name="papername" id="papername" class="form-control" title="Select Newspaper Name" onchange="select_price()" required>
                            <option value="" disabled selected>Select</option>';
                            if($result = $mysqli->query($qury)) {
                                if(mysqli_num_rows($result)>0) {
                                  while($row = $result->fetch_assoc()) {
                                        array_push($regular_price,$row['regular_price']);
                                        array_push($sunday_price,$row['sunday_price']);
                                       echo '<option value="'.$row['newspaper_name'].'">'.$row['newspaper_name'].'</option>';
                                    }
                                }
                            }
                    echo '
                    ...
                    <div class="col-lg-2 col-md-4 pt-3 mb-1">
                        <label for="quentity" class="mb-0"><b>Newspaper Quentity</b><span class="text-danger">*</span></label>
                    ...
                    <div class="col-lg-2 col-md-4 pt-3 mb-1">
                        <label for="price" class="mb-0"><b>Newspaper Price</b><span class="text-danger">*</span></label>
                        <input type="number" name="price" id="price" class="form-control" placeholder="Enter Magazine Price"
                    ...
                    <div class="col-lg-2 col-md-4 pt-3 mb-1">
                        ...
                        <input type="number" name="percentage" id="percentage" class="form-control" placeholder="Enter Your Percentage"
                            title="Enter Your Percentage" onkeyup="total_amount()" required>
                    </div>
                    <div class="col-lg-2 col-md-4 pt-3 mb-1">
                        ...
                        <input type="text" name="total" id="total" class="form-control" placeholder="Enter Total Price"
                            title="Enter Total Price" onfocus="total_amount()" readonly required>
                    </div>
            ...
            <script>
                var regular_price = [];';
                $count = 0;
                while($count < count($regular_price)){
                    echo'regular_price.push('.$regular_price[$count].');';
                    $count=$count+1;
                }
                echo '
                var sunday_price = [];';
                $count = 0;
                while($count < count($sunday_price)){
                    echo'sunday_price.push('.$sunday_price[$count].');';
                    $count=$count+1;
                }
                echo '
                function select_price(){ 
                    var a = document.getElementById("date").value.split("-");
                    var today = new Date(a[0],a[1]-1,a[2]);
                    if(today.getDay() == 0 && sunday_price[document.getElementById("papername").selectedIndex-1] > 0)
                        document.getElementById("price").value = sunday_price[document.getElementById("papername").selectedIndex-1];
                    else
                        document.getElementById("price").value = regular_price[document.getElementById("papername").selectedIndex-1];
                } 
