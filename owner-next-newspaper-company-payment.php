<?php 
    ...
    $bill = 0;
    $qury = "SELECT * FROM newspaper_company_payment WHERE newspaper_name = '".$papername."' and payment_year = '".$year."' and payment_month = '".$month."'";
    if($result = $mysqli->query($qury)){
        if(mysqli_num_rows($result)<=0){
            $qury = "SELECT SUM(total) AS bill FROM today_total_newspaper WHERE newspaper_name = '".$papername."' and year(date) = '".$year."' and month(date) = '".$month."'";
            if($result = $mysqli->query($qury)){
                if(mysqli_num_rows($result)>0){
            ...
            $qury = "SELECT due_amount FROM newspaper_company_payment  WHERE newspaper_name = '".$papername."' and payment_year = '".$year."' and payment_month = '".$month."' order by id desc";
            if($result = $mysqli->query($qury)){
                if(mysqli_num_rows($result)>0){
                    $row = $result->fetch_assoc();
                    $bill = $row['due_amount'];
    ...
    echo '
        ...
        <br><br>
            ...
            <form action="owner-input-next-newspaper-company-payment.php" method="post">
