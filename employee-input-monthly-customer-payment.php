<?php   
    ...
    $qury = "SELECT * FROM monthly_bill WHERE customer_name = '".$name."' and mobile = '".$mobile."' and year(date) = '".$year."' and month(date) = '".$month."'";
    if($result=$mysqli->query($qury)){
        if(mysqli_num_rows($result)<=0){
            ...
            if($result=$mysqli->query($qury)){
                if(mysqli_num_rows($result)<=0){
                    header('location:employee-next-monthly-customer-payment.php');           
                }
                else{
