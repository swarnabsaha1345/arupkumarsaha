<?php  
    ...
    $newspapername = $_POST['papername'];
    $qury = "SELECT newspaper_name FROM newspaper_entry WHERE newspaper_name = '".$newspapername."'";
    if($result = $mysqli->query($qury)){
        if(mysqli_num_rows($result)>0){
            display($newspapername);
        }
    }

    function display($newspapername){
        require_once 'database-connection.php';
        $mysqli = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_DATABASE);
        $qury = "SELECT * FROM newspaper_entry WHERE newspaper_name = '".$newspapername."'";
        if($result = $mysqli->query($qury)){
            if(mysqli_num_rows($result)>0){
                ...
                echo '
                    ...
                    <head>
                        ...
                        <title>Newspaper Details Check</title>
                    ...
                    <body>
                        ...
                        <div class="row mx-5 p-3" style="background-color: rgb(255, 187, 0); border-radius: 25px;">
                            ...
                            <div class="col-lg-2 col-md-4 pt-3 mb-1">
                                <label for="newspapername" class="mb-0"><b>Newspaper Name</b></label>
                                <input type="text" name="newspapername" id="newspapername" class="form-control text-capitalize" value="'.$row['newspaper_name'].'" readonly>
                            </div>
                            <div class="col-lg-2 col-md-4 pt-3 mb-1">
                                <label for="regularprice" class="mb-0"><b>Regular Price</b></label>
                                <input type="text" name="regularprice" id="regularprice" class="form-control" value="'.$row['regular_price'].'" readonly>
                            </div>
                            <div class="col-lg-2 col-md-4 pt-3 mb-1">
                                <label for="sundayprice" class="mb-0"><b>Sunday Price</b></label>
                                <input type="text" name="sundayprice" id="sundayprice" class="form-control" value="'.$row['sunday_price'].'" readonly>
                            </div>
                        </div><br>   
                </body>
